...
<div class="min-h-screen bg-base-100 flex items-center justify-center p-4 relative overflow-hidden">
	...
	<div class="w-full max-w-2xl relative z-10">
		...
		<div class="text-center mb-8">
			<div class="inline-flex items-center gap-3 mb-4">
				<div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-lg shadow-primary/25">
					<svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-primary-content" viewBox="0 0 24 24" fill="currentColor">
			...
			<p class="text-base-content/60 text-sm">Beautiful analytics for your Jellyfin media server</p>
		</div>
		...
		<!-- Step Content Card -->
		<div class="card bg-base-200/60 backdrop-blur-xl shadow-2xl border border-base-300/50">
				...
				{#key stepKey}
					<div class="step-enter">
						...
						{:else if currentStep === 1}
							<StepAuth {wizardData} {onStepComplete} onBack={prevStep} />
						{:else if currentStep === 2}
							<StepLibraries {wizardData} {onStepComplete} onBack={prevStep} />
						{:else if currentStep === 3}
							<StepApiKeys {wizardData} {onStepComplete} onBack={prevStep} />
