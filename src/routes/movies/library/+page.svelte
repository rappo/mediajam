<script>
    ...
    onMount(async () => {
        try {
            const res = await fetch('/api/pages/movies/library');
    ...
    let genreChartData = $derived.by(() => {
        ...
        const result = top10.map(([name, c]) => ({ name: name.charAt(0).toUpperCase() + name.slice(1), ...c }));
...
<svelte:head>
    <title>Mediajam — Movie Library</title>
</svelte:head>
...
<div class="page-wrap max-w-6xl mx-auto">
    ...
    <div class="page-header">
        ...
        <a href="/movies" class="btn btn-ghost btn-sm">← Back to Movies</a>
    </div>
