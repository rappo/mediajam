                ...
                {#if externalRatings.length > 0}
                    <div class="hero-ratings-bar">
                        ...
                        {#if getRating('omdb_rt')}
                            ...
                            <a href="https://www.rottentomatoes.com/m/{data.movie.title.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '')}" target="_blank" rel="noopener" class="hero-link">
                                <ServiceIcon service="rottentomatoes" size="w-4 h-4" />
                                <span class="hero-link-text">{r.raw_value}</span>
                            </a>
                        {/if}
                        {#if getRating('omdb_metacritic')}
                            {@const r = getRating('omdb_metacritic')}
                            <a href="https://www.metacritic.com/movie/{data.movie.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}/" target="_blank" rel="noopener" class="hero-link">
                                <ServiceIcon service="metacritic" size="w-4 h-4" />
                                <span class="hero-link-text">{r.raw_value}</span>
                            </a>
                        {/if}
                        {#if getRating('tmdb')}
